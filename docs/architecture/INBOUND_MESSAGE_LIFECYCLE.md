...
**Established:** 2026-08-25 — Updated 2026-08-27 (WILD-04R-F5.1) — Updated 2026-08-27 (WILD-04R-F6)  
**Read alongside:**
...
> **CRITICAL:** This document describes what the code does **now**, after WILD-04R, WILD-04R-F1, WILD-04R-F2, WILD-04R-F3, WILD-04R-F5, WILD-04R-F5.1, and WILD-04R-F6. If code conflicts with this document, STOP and report it as an architecture defect.
>
> **CRITICAL — WILD-04R-F6:** The LLM has NO authority to mutate `tipo_vehiculo` on a candidate whose vehicle identity is catalog-resolved. Once `marca+modelo` maps to a catalog entry, `tipo_vehiculo` is derived deterministically from that entry. AI proposals are overridden by `_catalog_tipo_for()` in `_apply_candidate()` before any write. Allowing LLM authority here caused a live pricing error: `SUV_4X4_DEPORTIVO → AUTO` on a location-only turn → $190,000 instead of $200,000.
...
| **WILD-04R-F5.1** | Deterministic required-next-info gate. Root cause: F5 D1 (AI prompt Rule 20) is LLM-guidance only — correctness cannot depend on LLM compliance. In the F4 live failure, `_routing_gate` Priority 3 (`_is_general_faq_or_soft_close`) intercepted the mixed-intent burst and set `skip_fallback=True`, bypassing `_check_fallback_flow_triggers` which would have sent the deterministic location question. Fix: `_apply_required_next_question()` deterministic finalizer added inside `_send_text_to_wa()`, after `_compose_secondary_answers()`. Gate conditions: QUALIFYING stage + not needs_human + focus candidate with tipo_vehiculo + location unknown + reply does not already ask for location + current turn is not a pure soft-close without inspection intent. Canonical append: `"\n\n¿En qué localidad o barrio está el auto?"`. Soft-close guard: if `_is_general_faq_or_soft_close(turn_text)` AND NOT `_detect_explicit_inspection_request(turn_text)` → gate is no-op (customer is leaving, not engaging commercially). AI prompt Rule 20 retained as guidance but correctness is now deterministic. Test suite: 28 invariant tests in `tests/test_wild04r_f5_1_required_location_gate.py` (probe unit tests, gate unit tests, exact live burst Case A + Case B). |
| **WILD-04R-F6** | Catalog authority for `tipo_vehiculo`. Live failure: Turn 2 "El auto está en San Miguel." — AI proposed `action=update, tipo_vehiculo=AUTO` on a Peugeot 2008 candidate (catalog type: `SUV_4X4_DEPORTIVO`). `_tipo_compatible(SUV_4X4_DEPORTIVO, AUTO)` returned True (same broad class), so `_apply_candidate` accepted the update and wrote `tipo=AUTO`. `PricingService` priced `AUTO + San Miguel viatico = 140000+50000 = $190,000` (wrong). Correct: `SUV_4X4_DEPORTIVO + San Miguel = 150000+50000 = $200,000`. Turn 3 acceptance was then blocked because the AI re-extracted `SUV_4X4_DEPORTIVO` from context, the vehicle-change guard fired (AUTO→SUV change), stage reset QUOTED→QUALIFYING, and no booking could proceed. Root cause: AI was allowed to overwrite a catalog-validated `tipo_vehiculo` with its own NLP guess from message text. Fix: `_catalog_tipo_for(marca, modelo)` private method added to CE — uses `_FULL_FORM_TO_ENTRY` + `_norm` (same as `_resolve_fuzzy_key`) to look up the catalog-authoritative tipo. In `_apply_candidate()` update path: if AI proposes a `tipo_vehiculo`, the effective marca+modelo is resolved against the catalog first; if a catalog entry exists, its tipo overrides the AI proposal. Same guard added to create path. Unknown vehicles (no catalog hit) fall through to AI tipo as before. Test suite: 10 test cases + 7 unit tests in `tests/test_wild04r_f6_tipo_authority.py`. Authority chain: customer language → vehicle identity (marca+modelo) → VehicleCatalog → canonical `tipo_vehiculo` → PricingService. |
